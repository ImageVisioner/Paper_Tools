function [] = draw(ir,vis,line_position)
    % 检查图像是否具有相同的尺寸
    ...
    % 检查线位置是否在图像范围内
    if line_position < 1 || line_position > size(ir, 2)
    ...
    imshow(ir);
    hold on;
    line([line_position, line_position], [1, size(ir,2)], 'Color', 'blue', 'LineWidth', 2);
    hold off;
    ...
    imshow(vis);
    hold on;
    line([line_position, line_position], [1, size(vis,2)], 'Color', 'red', 'LineWidth', 2);
    hold off;
